repeat wait() until game:IsLoaded()

if game.CreatorId == 4372130 then --Blox fruits 
    if hookmetamethod and require then
        loadstring(game:HttpGet("https://api.luarmor.net/files/v3/loaders/e2718ddebf562c5c4080dfce26b09398.lua"))()
    else
        loadstring(game:HttpGet("https://api.luarmor.net/files/v3/loaders/f1d09e6c5ebfe2ed922a61c4bfba05d3.lua"))()
    end
elseif game.CreatorId == 4294803 then -- Demonfall 
    loadstring(game:HttpGet("https://api.luarmor.net/files/v3/loaders/9b64d07193c7c2aef970d57aeb286e70.lua"))()
elseif game.CreatorId == 1226491 then -- Shindo Life
    loadstring(game:HttpGet("https://api.luarmor.net/files/v3/loaders/fefdf5088c44beb34ef52ed6b520507c.lua"))()
elseif game.CreatorId == 2613928 then -- Counter Blox
    loadstring(game:HttpGet("https://api.luarmor.net/files/v3/loaders/0e0166cb8f3c5c189c6a1f2532db2125.lua"))()
elseif game.CreatorId == 3229308 then -- Grand Piece Online
    loadstring(game:HttpGet("https://api.luarmor.net/files/v3/loaders/4b2841eb3e1345dd1aa50da164ca2bbe.lua"))()
elseif game.CreatorId == 7381705 then -- Fisch
    loadstring(game:HttpGet("https://api.luarmor.net/files/v3/loaders/8bbc8a7c4e023bc0c80799fec3233162.lua"))()
elseif game.CreatorId == 15557357 then -- Jujutsu Infinite
    loadstring(game:HttpGet("https://api.luarmor.net/files/v3/loaders/4fe2dfc202115670b1813277df916ab2.lua"))()
elseif game.CreatorId == 1848960 then -- Murder Mystery 2
    loadstring(game:HttpGet("https://api.luarmor.net/files/v3/loaders/9c916252a9f79bbf5a108f97c531e807.lua"))()
elseif game.CreatorId == 3959677 then -- Pet Simulator 99
    loadstring(game:HttpGet("https://api.luarmor.net/files/v3/loaders/e95ef6f27596e636a7d706375c040de4.lua"))()
elseif game.CreatorId == 3461453 then -- Rivals
    loadstring(game:HttpGet("https://api.luarmor.net/files/v3/loaders/3bb7969a9ecb9e317b0a24681327c2e2.lua"))()
elseif game.CreatorId == 17219742 then -- anime vanguards
    loadstring(game:HttpGet("https://api.luarmor.net/files/v3/loaders/25116432b45b1a5ea7811c264a731941.lua"))()
elseif game.CreatorId == 3059674 then -- Jailbreak
    loadstring(game:HttpGet("https://api.luarmor.net/files/v3/loaders/21ad7f491e4658e9dc9529a60c887c6e.lua"))()
end
